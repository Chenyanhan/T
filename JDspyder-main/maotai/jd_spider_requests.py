...
class JdSeckill(object):
    ...
    def _seckill(self):
        ...
        while True:
            try:
                self.request_seckill_url()#返回结算页面
                while True:
    ...
    def get_seckill_url(self):
        ...
        while True:
            resp = self.session.get(url=url, headers=headers, params=payload)
            resp_json = parse_json(resp.text)
            logger.info("抢购链接接口返回: %s", resp_json)
